...
Fragment brush_fs(){
    ...
    if(vFormat == 1){
        ...
        yuv_value . x = texture(sColor0, vec3(uv_y, vUv_Y . z). xy). r;
        yuv_value . y = texture(sColor1, vec3(uv_u, vUv_U . z). xy). r;
        yuv_value . z = texture(sColor2, vec3(uv_v, vUv_V . z). xy). r;
    } else if(vFormat == 0){
        vec2 uv_y = clamp(vUv_Y . xy, vUvBounds_Y . xy, vUvBounds_Y . zw);
        vec2 uv_uv = clamp(vUv_U . xy, vUvBounds_U . xy, vUvBounds_U . zw);
        yuv_value . x = texture(sColor0, vec3(uv_y, vUv_Y . z). xy). r;
        yuv_value . yz = texture(sColor1, vec3(uv_uv, vUv_U . z). xy). rg;
    } else if(vFormat == 2){



        vec2 uv_y = clamp(vUv_Y . xy, vUvBounds_Y . xy, vUvBounds_Y . zw);
        yuv_value = texture(sColor0, vec3(uv_y, vUv_Y . z). xy). gbr;
    } else {
        yuv_value = vec3(0.0);
    }
