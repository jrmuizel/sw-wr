...
vec4 vClipMaskUv
I32 vGradientAddress
Float vGradientRepeat
vec2 vScaledDir
vec2 vStartPoint
vec2 vRepeatedSize
vec2 vPos
...
vec2 vTileRepeat
*/
struct brush_linear_gradientDITHERING_program : ProgramImpl {
const char *get_name() const override { return "brush_linear_gradientDITHERING"; }
int get_uniform(const char *name) const override {
 if (strcmp("sDither", name) == 0) { return 8; }
...
struct brush_linear_gradientDITHERING_vert : VertexShaderImpl {
typedef brush_linear_gradientDITHERING_vert Self;
...
static void load_attribs(Self *self, brush_linear_gradientDITHERING_program *prog, VertexAttrib *attribs, unsigned short *indices, int start, int instance, int count) {
...
int32_t vGradientAddress;
float vGradientRepeat;
vec2_scalar vScaledDir;
vec2_scalar vStartPoint;
vec2_scalar vRepeatedSize;
vec2_scalar vTileRepeat;
};
...
ALWAYS_INLINE void store_flat_outputs(char* dest_ptr) {
  ...
  dest->vGradientAddress = vGradientAddress;
  dest->vGradientRepeat = vGradientRepeat;
  dest->vScaledDir = vScaledDir;
  dest->vStartPoint = vStartPoint;
  dest->vRepeatedSize = vRepeatedSize;
  dest->vTileRepeat = vTileRepeat;
}
...
static void bind_textures(Self *self, brush_linear_gradientDITHERING_program *prog) {
...
Gradient_scalar() = default;
Gradient_scalar(vec4_scalar start_end_point, int32_t extend_mode, vec2_scalar stretch_size) : start_end_point(start_end_point), extend_mode(extend_mode), stretch_size(stretch_size){}
};
struct Gradient {
...
vec2 stretch_size;
Gradient() = default;
Gradient(vec4 start_end_point, I32 extend_mode, vec2 stretch_size) : start_end_point(start_end_point), extend_mode(extend_mode), stretch_size(stretch_size){}
Gradient(vec4_scalar start_end_point, int32_t extend_mode, vec2_scalar stretch_size):start_end_point(start_end_point),extend_mode(extend_mode),stretch_size(stretch_size){
}
Gradient(Gradient_scalar s):start_end_point(s.start_end_point),extend_mode(s.extend_mode),stretch_size(s.stretch_size){
}
friend Gradient if_then_else(I32 c, Gradient t, Gradient e) { return Gradient(
if_then_else(c, t.start_end_point, e.start_end_point), if_then_else(c, t.extend_mode, e.extend_mode), if_then_else(c, t.stretch_size, e.stretch_size));
}};
...
Gradient fetch_gradient(I32 address) {
 std::array<vec4,2> data = fetch_from_gpu_cache_2(address);
 return Gradient(data[0], make_int((data[1]).sel(X)), (data[1]).sel(Y, Z));
}
void brush_vs(VertexInfo vi, I32 prim_address, RectWithSize local_rect, RectWithSize segment_rect, ivec4 prim_user_data, I32 segment_user_data, mat4 transform, PictureTask pic_task, I32 brush_flags, vec4 texel_rect) {
 Gradient gradient = fetch_gradient(prim_address);
 auto _c8_ = ((brush_flags)&(2))!=(0);
 {
  vPos = if_then_else(_c8_,(((vi).local_pos)-((segment_rect).p0))/((segment_rect).size),vPos);
  vPos = if_then_else(_c8_,((vPos)*(((texel_rect).sel(Z, W))-((texel_rect).sel(X, Y))))+((texel_rect).sel(X, Y)),vPos);
  vPos = if_then_else(_c8_,(vPos)*((local_rect).size),vPos);
 }
 {
  vPos = if_then_else(~(_c8_),((vi).local_pos)-((local_rect).p0),vPos);
 }
 vec2 start_point = ((gradient).start_end_point).sel(X, Y);
 vec2 end_point = ((gradient).start_end_point).sel(Z, W);
 vec2 dir = (end_point)-(start_point);
 vStartPoint = force_scalar(start_point);
 vScaledDir = force_scalar((dir)/(dot(dir, dir)));
 vec2 tile_repeat = ((local_rect).size)/((gradient).stretch_size);
 vRepeatedSize = force_scalar((gradient).stretch_size);
 vGradientAddress = force_scalar((prim_user_data).sel(X));
 vGradientRepeat = force_scalar(make_float(((gradient).extend_mode)!=(0)));
 vTileRepeat = force_scalar(tile_repeat);
...
ALWAYS_INLINE void main(void) {
 ...
 {
  I32 segment_address = ((ph).specific_prim_address)+((2)+((segment_index)*(2)));
 ...
 auto _c4_ = (transform).is_axis_aligned;
 {
  ...
  init_transform_vs(make_vec4(make_vec2(-(10000000000000000.f)), make_vec2(10000000000000000.f)), _c4_);
 }
 {
...
vec4 oFragColor
*/
struct brush_linear_gradientDITHERING_frag : FragmentShaderImpl {
typedef brush_linear_gradientDITHERING_frag Self;
static void set_uniform_1i(Self *self, int index, int value) {
 switch (index) {
 case 8:
  assert(0); // sDither
...
typedef brush_linear_gradientDITHERING_vert::FlatOutputs FlatInputs;
typedef brush_linear_gradientDITHERING_vert::InterpOutputs InterpInputs;
static void read_flat_inputs(Self *self, const FlatInputs *src) {
  ...
  self->vGradientAddress = src->vGradientAddress;
  self->vGradientRepeat = src->vGradientRepeat;
  self->vScaledDir = src->vScaledDir;
  self->vStartPoint = src->vStartPoint;
  self->vRepeatedSize = src->vRepeatedSize;
  self->vTileRepeat = src->vTileRepeat;
}
...
static void bind_textures(Self *self, brush_linear_gradientDITHERING_program *prog) {
 self->sDither = lookup_sampler(&prog->samplers.sDither_impl, prog->samplers.sDither_slot);
 self->sGpuCache = lookup_sampler(&prog->samplers.sGpuCache_impl, prog->samplers.sGpuCache_slot);
...
vec4 dither(vec4 color) {
 int32_t const matrix_mask = 7;
 ivec2 pos = (make_ivec2((gl_FragCoord).sel(X, Y)))&(make_ivec2(matrix_mask));
 Float noise_normalized = ((((texelFetch(sDither, pos, 0)).sel(R))*(255.f))+(0.5f))/(64.f);
 Float noise = ((noise_normalized)-(0.5f))/(256.f);
 return (color)+(make_vec4(noise, noise, noise, 0));
}
vec4 sample_gradient(int32_t address, Float offset, float gradient_repeat) {
 Float x = mix(offset, fract(offset), gradient_repeat);
 int32_t const GRADIENT_ENTRIES = 128;
 x = (1.f)+((x)*(make_float(GRADIENT_ENTRIES)));
 I32 lut_offset = (2)*(make_int(floor(x)));
 lut_offset = clamp(lut_offset, 0, (2)*((GRADIENT_ENTRIES)+(1)));
 std::array<vec4,2> texels = fetch_from_gpu_cache_2((address)+(lut_offset));
...
Float distance_aa(Float aa_range, Float signed_distance) {
 ...
 auto _c7_ = (dist)<=((-(0.5f))+(0.0001f));
 ret = 1.f;
 ret_mask = ~I32(_c7_);
 auto _c8_ = (dist)>=((0.5f)-(0.0001f));
 ret = if_then_else(ret_mask & I32(_c8_), 0.f, ret);
 ret_mask &= ~I32(_c8_);
...
Fragment brush_fs() {
 vec2 local_pos = max(vPos, make_vec2(0.f));
 vec2 pos = mod(local_pos, vRepeatedSize);
 vec2_scalar prim_size = (vRepeatedSize)*(vTileRepeat);
 auto _c3_ = ((local_pos).sel(X))>=((prim_size).sel(X));
 {
  (pos).sel(X) = if_then_else(_c3_,(vRepeatedSize).sel(X),(pos).sel(X));
 }
 auto _c4_ = ((local_pos).sel(Y))>=((prim_size).sel(Y));
 {
  (pos).sel(Y) = if_then_else(_c4_,(vRepeatedSize).sel(Y),(pos).sel(Y));
 }
 Float offset = dot((pos)-(vStartPoint), vScaledDir);
 vec4 color = sample_gradient(vGradientAddress, offset, vGradientRepeat);
...
Float do_clip() {
 ...
 auto _c10_ = !(all(make_bvec4(left, right)));
 {
  ret = 0.f;
  ret_mask = ~I32(_c10_);
 }
...
void brush_linear_gradientDITHERING_program::init_shaders(void *vertex_shader, void *fragment_shader) {
 reinterpret_cast<brush_linear_gradientDITHERING_vert*>(vertex_shader)->init_shader();
 reinterpret_cast<brush_linear_gradientDITHERING_frag*>(fragment_shader)->init_shader();
}
