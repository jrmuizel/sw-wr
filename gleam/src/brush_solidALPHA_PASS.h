...
vec2 vLocalPos
*/
struct brush_solidALPHA_PASS_program : ProgramImpl {
const char *get_name() const override { return "brush_solidALPHA_PASS"; }
...
struct brush_solidALPHA_PASS_vert : VertexShaderImpl {
typedef brush_solidALPHA_PASS_vert Self;
...
static void load_attribs(Self *self, brush_solidALPHA_PASS_program *prog, VertexAttrib *attribs, unsigned short *indices, int start, int instance, int count) {
...
static void bind_textures(Self *self, brush_solidALPHA_PASS_program *prog) {
...
vec4 oFragColor
*/
struct brush_solidALPHA_PASS_frag : FragmentShaderImpl {
typedef brush_solidALPHA_PASS_frag Self;
static void set_uniform_1i(Self *self, int index, int value) {
 switch (index) {
 case 2:
  assert(0); // sGpuCache
...
typedef brush_solidALPHA_PASS_vert::FlatOutputs FlatInputs;
typedef brush_solidALPHA_PASS_vert::InterpOutputs InterpInputs;
...
Float do_clip() {
 ...
 auto _c6_ = !(all(make_bvec4(left, right)));
 {
  ret = 0.f;
  ret_mask = ~I32(_c6_);
 }
...
void brush_solidALPHA_PASS_program::init_shaders(void *vertex_shader, void *fragment_shader) {
 reinterpret_cast<brush_solidALPHA_PASS_vert*>(vertex_shader)->init_shader();
 reinterpret_cast<brush_solidALPHA_PASS_frag*>(fragment_shader)->init_shader();
}
