...
Float vClipMode
*/
struct cs_clip_rectangleFAST_PATH_program : ProgramImpl {
const char *get_name() const override { return "cs_clip_rectangleFAST_PATH"; }
...
struct cs_clip_rectangleFAST_PATH_vert : VertexShaderImpl {
typedef cs_clip_rectangleFAST_PATH_vert Self;
...
static void load_attribs(Self *self, cs_clip_rectangleFAST_PATH_program *prog, VertexAttrib *attribs, unsigned short *indices, int start, int instance, int count) {
...
static void bind_textures(Self *self, cs_clip_rectangleFAST_PATH_program *prog) {
...
ALWAYS_INLINE void main(void) {
 ...
 vec2 half_size = (0.5f)*((local_rect).size);
 Float radius = (((clip).top_left).outer_inner_radius).sel(X);
 vLocalPos = (((vi).local_pos).sel(X, Y))-((half_size)-((cmi).local_pos));
 (vClipParams).lsel(X, Y) = force_scalar((half_size)-(make_vec2(radius)));
 (vClipParams).sel(Z) = force_scalar(radius);
}
...
vec4 oFragColor
*/
struct cs_clip_rectangleFAST_PATH_frag : FragmentShaderImpl {
typedef cs_clip_rectangleFAST_PATH_frag Self;
static void set_uniform_1i(Self *self, int index, int value) {
 switch (index) {
 case 2:
  assert(0); // sGpuCache
...
typedef cs_clip_rectangleFAST_PATH_vert::FlatOutputs FlatInputs;
typedef cs_clip_rectangleFAST_PATH_vert::InterpOutputs InterpInputs;
static void read_flat_inputs(Self *self, const FlatInputs *src) {
  self->vTransformBounds = src->vTransformBounds;
  self->vClipParams = src->vClipParams;
...
Float sdf_rounded_rect(vec2 pos, vec3_scalar clip_params) {
 return (length(max((abs(pos))-((clip_params).sel(X, Y)), 0.f)))-((clip_params).sel(Z));
}
...
void cs_clip_rectangleFAST_PATH_program::init_shaders(void *vertex_shader, void *fragment_shader) {
 reinterpret_cast<cs_clip_rectangleFAST_PATH_vert*>(vertex_shader)->init_shader();
 reinterpret_cast<cs_clip_rectangleFAST_PATH_frag*>(fragment_shader)->init_shader();
}
