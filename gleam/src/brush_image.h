...
void read_flat_inputs(const void* src_ptr) {
  ...
  vLayerAndPerspective = src->vLayerAndPerspective;
  vUvBounds = src->vUvBounds;
  vUvSampleBounds = src->vUvSampleBounds;
}
void read_interp_inputs(const void* init_ptr, const void* step_ptr) {
  ...
  vUv = init_interp(init->vUv, step->vUv);
}
...
void bind_attrib(const char *name, int index) override {
 brush_image_vert::bind_attrib_location(name, index);
}
