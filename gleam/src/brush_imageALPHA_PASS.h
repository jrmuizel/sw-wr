...
vec2 vTileRepeat
*/
struct brush_imageALPHA_PASS_program : ProgramImpl {
const char *get_name() const override { return "brush_imageALPHA_PASS"; }
int get_uniform(const char *name) const override {
 ...
 if (strcmp("uMode", name) == 0) { return 8; }
...
struct brush_imageALPHA_PASS_vert : VertexShaderImpl {
typedef brush_imageALPHA_PASS_vert Self;
...
static void load_attribs(Self *self, brush_imageALPHA_PASS_program *prog, VertexAttrib *attribs, unsigned short *indices, int start, int instance, int count) {
...
static void bind_textures(Self *self, brush_imageALPHA_PASS_program *prog) {
...
std::array<vec4,4> fetch_from_gpu_cache_4(I32 address) {
 ivec2 uv = get_gpu_cache_uv(address);
 return std::array<vec4,4>{texelFetch(sGpuCache, (uv)+(make_ivec2(0, 0)), 0), texelFetch(sGpuCache, (uv)+(make_ivec2(1, 0)), 0), texelFetch(sGpuCache, (uv)+(make_ivec2(2, 0)), 0), texelFetch(sGpuCache, (uv)+(make_ivec2(3, 0)), 0)};
}
...
void brush_vs(VertexInfo vi, I32 prim_address, RectWithSize prim_rect, RectWithSize segment_rect, ivec4 prim_user_data, I32 segment_user_data, mat4 transform, PictureTask pic_task, I32 brush_flags, vec4 segment_data) {
 ...
 auto _c25_ = ((stretch_size).sel(X))<(0.f);
 {
  stretch_size = if_then_else(_c25_,(local_rect).size,stretch_size);
 }
 auto _c26_ = ((brush_flags)&(2))!=(0);
 {
  local_rect = if_then_else(_c26_,segment_rect,local_rect);
  stretch_size = if_then_else(_c26_,(local_rect).size,stretch_size);
    auto _c27_ = (_c26_)&(((brush_flags)&(4))!=(0));
  {
   (stretch_size).sel(X) = if_then_else(_c27_,((segment_data).sel(Z))-((segment_data).sel(X)),(stretch_size).sel(X));
  }
    auto _c28_ = (_c26_)&(((brush_flags)&(8))!=(0));
  {
   (stretch_size).sel(Y) = if_then_else(_c28_,((segment_data).sel(W))-((segment_data).sel(Y)),(stretch_size).sel(Y));
  }
    auto _c29_ = (_c26_)&(((brush_flags)&(16))!=(0));
  {
   vec2 uv_size = (((res).uv_rect).p1)-(((res).uv_rect).p0);
   uv0 = if_then_else(_c29_,(((res).uv_rect).p0)+(((segment_data).sel(X, Y))*(uv_size)),uv0);
   uv1 = if_then_else(_c29_,(((res).uv_rect).p0)+(((segment_data).sel(Z, W))*(uv_size)),uv1);
      auto _c30_ = (_c29_)&(((brush_flags)&(4))!=(0));
   {
    (stretch_size).sel(X) = if_then_else(_c30_,((stretch_size).sel(X))*((uv_size).sel(X)),(stretch_size).sel(X));
   }
      auto _c31_ = (_c29_)&(((brush_flags)&(8))!=(0));
   {
    (stretch_size).sel(Y) = if_then_else(_c31_,((stretch_size).sel(Y))*((uv_size).sel(Y)),(stretch_size).sel(Y));
   }
  }
 }
 ...
 I32 color_mode = ((prim_user_data).sel(X))&(65535);
 I32 blend_mode = ((prim_user_data).sel(X))>>(16);
 I32 raster_space = (prim_user_data).sel(Y);
 auto _c32_ = (color_mode)==(0);
 {
  color_mode = if_then_else(_c32_,uMode,color_mode);
 }
 auto _c33_ = (raster_space)==(1);
 {
  f = if_then_else(_c33_,get_image_quad_uv(segment_user_data, f),f);
 }
 if (true) {
  {
  }
 ...
 vUv /= texture_size;
 vUv *= (repeat).sel(X, Y);
 auto _c34_ = (perspective_interpolate)==(0.f);
 {
  vUv = if_then_else(_c34_,vUv*((vi).world_pos).sel(W),vUv);
 }
 vUvBounds = force_scalar((make_vec4(min_uv, max_uv))/((texture_size).sel(X, Y, X, Y)));
 vTileRepeat = force_scalar((repeat).sel(X, Y));
 Float opacity = (make_float((prim_user_data).sel(Z)))/(65535.f);
 auto _c35_ = (blend_mode)==(0);
 {
  ((image_data).color).sel(A) = if_then_else(_c35_,((image_data).color).sel(A)*opacity,((image_data).color).sel(A));
 }
 if (true) {
  {
   (image_data).color = if_then_else(~(_c35_),(image_data).color*opacity,(image_data).color);
  }
 }
 auto _c36_ = ((color_mode)==(1))||((color_mode)==(7));
 {
  if (_c36_[0]) { vMaskSwizzle = make_vec2(0.f, 1.f); };
  if (_c36_[0]) { vColor = force_scalar((image_data).color); };
 }
 auto _c37_ = (((color_mode)==(5))||((color_mode)==(6)))||((color_mode)==(9));
 auto _c38_ = (~(_c36_))&(_c37_);
 {
  if (_c38_[0]) { vMaskSwizzle = make_vec2(1.f, 0.f); };
  if (_c38_[0]) { vColor = force_scalar((image_data).color); };
 }
 _c37_ = (~(_c36_))&(~(_c37_));
 auto _c39_ = (((color_mode)==(2))||((color_mode)==(3)))||((color_mode)==(8));
 auto _c40_ = (_c37_)&(_c39_);
 {
  if (_c40_[0]) { vMaskSwizzle = make_vec2(1.f, 0.f); };
  if (_c40_[0]) { vColor = force_scalar(make_vec4(((image_data).color).sel(A))); };
 }
 _c39_ = (_c37_)&(~(_c39_));
 auto _c41_ = (color_mode)==(4);
 auto _c42_ = (_c39_)&(_c41_);
 {
  if (_c42_[0]) { vMaskSwizzle = make_vec2(-(1.f), 1.f); };
  if (_c42_[0]) { vColor = force_scalar((make_vec4(((image_data).color).sel(A)))*((image_data).background_color)); };
 }
 _c41_ = (_c39_)&(~(_c41_));
 if (true) {
  {
   if (_c41_[0]) { vMaskSwizzle = make_vec2(0.f); };
   if (_c41_[0]) { vColor = make_vec4(1.f); };
  }
 }
...
vec4 oFragColor
*/
struct brush_imageALPHA_PASS_frag : FragmentShaderImpl {
typedef brush_imageALPHA_PASS_frag Self;
static void set_uniform_1i(Self *self, int index, int value) {
 switch (index) {
 case 7:
  assert(0); // sColor0
...
typedef brush_imageALPHA_PASS_vert::FlatOutputs FlatInputs;
typedef brush_imageALPHA_PASS_vert::InterpOutputs InterpInputs;
...
Fragment brush_fs() {
 ...
 Fragment frag;
 float alpha = 1.f;
 (texel).lsel(R, G, B) = (((texel).sel(R, G, B))*((vMaskSwizzle).sel(X)))+(((texel).sel(A, A, A))*((vMaskSwizzle).sel(Y)));
...
void brush_imageALPHA_PASS_program::init_shaders(void *vertex_shader, void *fragment_shader) {
 reinterpret_cast<brush_imageALPHA_PASS_vert*>(vertex_shader)->init_shader();
 reinterpret_cast<brush_imageALPHA_PASS_frag*>(fragment_shader)->init_shader();
}
